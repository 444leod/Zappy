...
void gui::GameDisplay::initialize(ILibrary& lib)
{
    ...
    spec.graphical = gui::TextureImage{
        .path = "./image.png",
        .subrect = gui::Rect<uint32_t>{8070, 2014, 298, 366}
    };
...
void gui::GameDisplay::draw(ILibrary& lib, [[__maybe_unused__]] GameData& data)
{
    lib.display().clear(gui::Color{0, 0, 0, 0});
    lib.display().draw(lib.textures().get("map_tile"), 100, 100, 1);
    lib.display().flush();
